...
boolean TryWalk (objtype *ob)
{
    ...
    if (ob->obclass == inertobj)
    {
        ...
        {
        case north:
            if (ob->obclass == dogobj || ob->obclass == fakeobj || ob->obclass == companionobj)
            {
...
//
    ...
    // TODO Possibly implement chasing of enemies here
    case companionobj:
        PlaySoundLocActor(DOGBARKSND,ob);
    ...
    // TODO Add flag for companion attackmode
    // Do no mark attack mode for companion for now
    if (ob->obclass != companionobj)
